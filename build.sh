#!/bin/bash
# Docker Buildx Build Script for routinecheck
...
Build Docker image for $routinecheck using buildx.
...
echo -e "${BLUE}📦 Building $routinecheck...${NC}"
...
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$routinecheck:latest"
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$routinecheck:$TIMESTAMP"
...
if eval "$BUILD_CMD"; then
    echo ""
    echo -e "${GREEN}✅ Successfully built $routinecheck${NC}"
    echo -e "  Image: $REGISTRY/$routinecheck:latest"
    echo -e "  Image: $REGISTRY/$routinecheck:$TIMESTAMP"
else
    echo ""
    echo -e "${RED}❌ Failed to build $routinecheck${NC}"
    exit 1
fi
